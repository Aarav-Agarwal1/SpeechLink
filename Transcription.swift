...
final class SpokenWordTranscriber: Sendable {
    ...
    static let magenta = Color(red: 0.54, green: 0.02, blue: 0.6).opacity(0.8) // #e81cff, color for volatile text
    ...
    var conversation: Conversation//connects to conversation, used for live updates
            ...
            do {
                for try await case let result in transcriber.results {
                    ...
                    if result.isFinal {
                        ...
                        volatileTranscript = ""
                        updateConversationWithNewText(withFinal: text)
                    } else {
    ...
    func updateConversationWithNewText(withFinal str: AttributedString) {
        conversation.text.append(str)
    }
...
extension SpokenWordTranscriber {
    ...
    func deallocate() async {
        // No-op: Explicit deallocation is not available in this SDK.
    }
}
