...
public enum TranscriptionError: Error {//cases with how it could go wrong
...
extension Recorder {
    ...
    func writeBufferToDisk(buffer: AVAudioPCMBuffer) {
        guard let file else {
            print("file is nil when writing")
            return
        }
        ...
        do {
            ...
            print("Failed to convert or write buffer to disk: \(error)")
        }
    }
